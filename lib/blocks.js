...
BlockController.prototype.transformInvBlock = function(block) {
  var self = this;
  ...
  var time = new Date(blockObj.header.time * 1000).toISOString(false);
  ...
  var transformed = {
    ...
    payload: {
      ...
      block: {
        hash: block.hash,
        previous_block_hash: blockObj.header.prevHash,
        height: info.height,
        confirmations: this.node.services.db.tip.__height - info.height + 1,
        merkle_root: blockObj.header.merkleRoot,
        time: time,
        nonce: blockObj.header.nonce,
        difficulty: block.header.getDifficulty(),
        transaction_hashes: transactionIds
      }
    }
  }
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  var info = this.node.services.bitcoind.getBlockIndex(height);
  if(!info) {
    return common.handleErrors(null, res);
  }
...
BlockController.prototype.reorg = function(req, res) {
  var self = this;
  ...
  self.node.getBlock(blockHash, function(err, block) {
    ...
    var isMainChain = self.node.services.bitcoind.isMainChain(blockHash);
    if (isMainChain) {
      res.jsonp({isMainChain: true});
    } else {
      self.node.services.db.findCommonAncestor(blockHash, function(err, block){
        res.jsonp({
          isMainChain: false,
          commonAncestor: block.hash
        });
      });
    }
  });
...
BlockController.prototype.currentBlock = function(req, res, next) {
  var self = this;
  console.log("HERE")
  req.params.blockHash = this.node.services.db.tip.hash;
  next();
};
