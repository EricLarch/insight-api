...
TxController.prototype.multishow = function(req, res) {
  var self = this;
  ...
  async.map(
    ...
    function(txid, next) {
      self.node.getTransactionWithBlockInfo(txid, true, function(err, transaction) {
        if(transaction.id == "d21633ba23f70118185227be58a63527675641ad37967e2aa461559f577aec43") { //SHITTY FIX FOR TX NOT FOUND
...
TxController.prototype.transformTransaction = function(transaction, withConfirmations, callback) {
  ...
  transformed.lock_time = txObj.nLockTime;
  transformed.block = null;
  ...
  async.map(
    ...
    function(err, outputs) {
      ...
      if (transaction.__height >=0 && transaction.__blockHash != "") {
        transformed.block = {
          hash: transaction.__blockHash,
          height: b_height,
          time: time.toISOString(false)
        };
      }
