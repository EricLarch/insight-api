...
TxController.prototype.transformInvTransaction = function(transaction) {
  ...
  return this.node.getTransactionWithBlockInfo(txid, true, function(err, transaction) {
    if (err) {
      console.warn("debug error"+err.toString());
    }

    transaction.populateInputs(self.node.services.db, [], function(err) {
      if(err) {
        console.warn("debug error 2 "+err.toString());
      }

      self.transformTransaction(transaction, function(err, transformedTransaction) {
        if (err) {
          console.log("debug error 3 "+err.toString());
        }
        ...
        for (var i = 0; i < txObj.outputs.length; i++) {
          ...
          if (script) {
            var address = script.toAddress(self.node.network);
            if (address) {
              tranformedOutput.addresses = [address.toString()];
            }
          }

          tranformedOutput.script_hex = output.script

          outputs.push(tranformedOutput);
        ...
        if(transaction.isCoinbase()) {
          ...
          for (var i = 0; i < txObj.inputs.length; i++) {
            ...
            var transformedInput = {
              ...
              value: 0,
              addresses: null,
              script_signature: input.script
            }
            console.log(input)

            if(input.output) {
              valueIn += input.output.satoshis
              transformedInput.value = input.output.satoshis;
              transformedInput.addresses = [bitcore.Script(input.output.script).toAddress(self.node.network).toString()];
            }

            inputs.push(transformedInput);
          }
          if(transaction.hasAllUtxoInfo()) {
        ...
        inputs = txObj.inputs.map(self.transformInput.bind(self));
        ...
        return {
          ...
          payload: {
            ...
            transaction: {
              ...
              chain_received_at: time.toISOString(),
              confirmations: 0,
              inputs: inputs,
              outputs: outputs,
              fees: fees,
