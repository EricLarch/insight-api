  ...
  this.node.getTransactionWithBlockInfo(txid, true, function(err, transaction) {
    ...
    transaction.populateInputs(self.node.services.db, [], function(err) {
      ...
      self.transformTransaction(transaction, function(err, transformedTransaction) {
        ...
        var txObj = transformedTransaction.toObject();


        var time = transformedTransaction.__timestamp ? transformedTransaction.__timestamp * 1000 : Math.round(Date.now());
        time = new Date(time);
        ...
        var result = {
          ...
          payload: {
            ...
            transaction: {
              ...
              inputs: inputs,
              outputs: outputs,
              fees: transformedTransaction.fees,
