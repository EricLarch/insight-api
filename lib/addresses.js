...
AddressController.prototype.multitxsconfirmations = function(req, res, next) {
  var self = this;
  ...
  self.node.getAddressHistory(req.addrs, options, function(err, result) {
    ...
    self.transformAddressHistoryForMultiTxsConfirmations(result.items, function(err, items) {
      if (err) {
        return common.handleErrors(err, res);
      }
      res.jsonp({
...
AddressController.prototype.transformAddressHistoryForMultiTxsConfirmations = function(txinfos, callback) {
  var self = this;
  var items = txinfos.map(function(txinfo) {
    return txinfo.tx;
  }).filter(function(value, index, self) {
  ...
  async.map(
    ...
    function(item, next) {
      ...
      var conf_nb = 0;
      if(item.__height >= 0) {
        conf_nb = self.node.services.db.tip.__height - item.__height + 1;
      }
